<?php
class admin_kelompok_kejahatan extends admin_controller {

	var $controller ;

	function admin_kelompok_kejahatan(){
		parent::__construct();
		// $this->load->model("core_model","cm");
		$this->load->model("coremodel","cm");
		$this->load->helper("tanggal");
		$this->load->model("akk_model","dm");
	...
	function index(){
		...
		$this->set_subtitle("DATA KELOMPOK KEJAHATAN");
		$this->set_title("DATA KELOMPOK KEJAHATAN");
...
function simpan(){
		...
		if($this->form_validation->run() == TRUE ) { 
			...
			unset($data['id_kelompok']);
			 

			 
			 $res = $this->db->insert("m_kelompok_kejahatan",$data);
			 if($res) {
...
function update(){
		...
		if($this->form_validation->run() == TRUE ) { 
			 ...
			 $res = $this->db->update("m_kelompok_kejahatan",$data);
			 if($res) {
...
function hapus(){
	...
	$res = $this->db->delete("m_kelompok_kejahatan");
	if($res){
