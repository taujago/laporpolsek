<?php
class admin_lapas extends admin_controller {
	...
	function admin_lapas(){
		...
		$this->load->model("admin_lapas_model","dm");
	...
	function index(){
		...
		$this->set_subtitle("DATA LAPAS");
		$this->set_title("DATA LAPAS");
...
function simpan(){
		...
		$this->form_validation->set_rules('lapas','Nama lapas','required'); 
		...
		if($this->form_validation->run() == TRUE ) { 
			...
			unset($data['id']);
			 ...
			 $res = $this->db->insert("m_lapas",$data);
			 if($res) {
...
function update(){
		...
		if($this->form_validation->run() == TRUE ) { 
			 ...
			 $res = $this->db->update("m_lapas",$data);
			 if($res) {
