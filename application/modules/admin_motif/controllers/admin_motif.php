<?php
class admin_motif extends admin_controller {

	var $controller ;

	function admin_motif(){
		parent::__construct();
		// $this->load->model("core_model","cm");
		$this->load->model("coremodel","cm");
		$this->load->helper("tanggal");
		$this->load->model("admin_motif_model","dm");
	...
	function index(){
		...
		$this->set_subtitle("DATA MOTIF KEJAHATAN");
		$this->set_title("DATA MOTIF KEJAHATAN");
...
function simpan(){
		...
		$this->form_validation->set_rules('motif','Motif kejahatan','required'); 
		...
		if($this->form_validation->run() == TRUE ) { 
			...
			unset($data['id_motif']);
			 ...
			 $res = $this->db->insert("m_motif",$data);
			 if($res) {
...
function update(){
		...
		if($this->form_validation->run() == TRUE ) { 
			 ...
			 $res = $this->db->update("m_motif",$data);
			 if($res) {
...
function hapus(){
	$data = $this->input->post();
	$this->db->where("id_motif",$data['id']);
	$res = $this->db->delete("m_motif");
	if($res){
