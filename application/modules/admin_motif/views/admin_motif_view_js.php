...
BootstrapDialog.show({
            message : 'ANDA AKAN MENGHAPUS DATA MOTIF KEJAHATAN. ANDA YAKIN  ?  ',
            title: 'KONFIRMASI HAPUS DATA MOTIF KEJAHATAN',
            draggable: true,
            buttons : [
              {
                label : 'YA',
...
function baru(){
	...
	$("#titleModal").html('TAMBAH DATA MOTIF KEJAHATAN ');
}
...
$("#titleModal").html('EDIT DATA MOTIF KEJAHATAN');

	$.ajax({
		url : '<?php echo site_url("$controller/get_json_detail") ?>/'+id, 
		dataType : 'json',
		success : function(obj) {
			$("#id_motif").val(obj.id_motif);
			$("#motif").val(obj.motif);
