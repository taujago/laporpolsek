<?php
class admin_pengadilan extends admin_controller {

	var $controller ;

	function admin_pengadilan(){
		parent::__construct();
		// $this->load->model("core_model","cm");
		$this->load->model("coremodel","cm");
		$this->load->helper("tanggal");
		$this->load->model("admin_pengadilan_model","dm");
	...
	function index(){
		...
		$this->set_subtitle("DATA PENGADILAN");
		$this->set_title("DATA PENGADILAN");
...
function simpan(){
		...
		$this->form_validation->set_rules('pengadilan','Nama Pengadilan','required'); 
		...
		if($this->form_validation->run() == TRUE ) { 
			...
			unset($data['id']);
			 ...
			 $res = $this->db->insert("m_pengadilan",$data);
			 if($res) {
...
function update(){
		...
		if($this->form_validation->run() == TRUE ) { 
			 ...
			 $res = $this->db->update("m_pengadilan",$data);
			 if($res) {
...
function hapus(){
	$data = $this->input->post();
	$this->db->where("id",$data['id']);
	$res = $this->db->delete("m_pengadilan");
	if($res){
