<?php
class admin_polres extends admin_controller {

	var $controller ;

	function admin_polres(){
		parent::__construct();
		// $this->load->model("core_model","cm");
		$this->load->model("coremodel","cm");
		$this->load->helper("tanggal");
		$this->load->model("polres_model","dm");
...
function simpan(){
		...
		$this->form_validation->set_rules('nama_polres','Nama Polres','required'); 
...
function update(){
		...
		if($this->form_validation->run() == TRUE ) { 
			 ...
			 $res = $this->db->update("m_polres",$data);
			 if($res) {
...
function hapus(){
	$data = $this->input->post();
	$this->db->where("id_polres",$data['id']);
	$res = $this->db->delete("m_polres");
	if($res){
