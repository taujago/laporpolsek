<?php
class admin_tempat extends admin_controller {

	var $controller ;

	function admin_tempat(){
		parent::__construct();
		// $this->load->model("core_model","cm");
		$this->load->model("coremodel","cm");
		$this->load->helper("tanggal");
		$this->load->model("admin_tempat_model","dm");
	...
	function index(){
		...
		$this->set_subtitle("DATA KATEGORI TEMPAT KEJAHATAN");
		$this->set_title("DATA KATEGORI TEMPAT KEJAHATAN");
...
function simpan(){
		...
		$this->form_validation->set_rules('jenis_lokasi','Kategori tempat','required'); 
		...
		if($this->form_validation->run() == TRUE ) { 
			...
			unset($data['id_jenis_lokasi']);
			 

			 
			 $res = $this->db->insert("m_jenis_lokasi",$data);
			 if($res) {
...
function update(){
		...
		if($this->form_validation->run() == TRUE ) { 
			 ...
			 $this->db->where("id_jenis_lokasi",$data['id_jenis_lokasi']);
			 $res = $this->db->update("m_jenis_lokasi",$data);
			 if($res) {
...
function hapus(){
	$data = $this->input->post();
	$this->db->where("id_jenis_lokasi",$data['id']);
	$res = $this->db->delete("m_jenis_lokasi");
	if($res){
