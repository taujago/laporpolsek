<?php
class penyidik_laka extends penyidik_controller {
 	var $controller ;

	function penyidik_laka(){
		parent::__construct();
		 
		$this->load->model("coremodel","cm");
		$this->load->helper("tanggal");
		$this->load->model("admindik_laka_model","dm");
        $this->load->model("penyidik_laka_model","am");
...
function get_data_perkembangan($lap_laka_lantas_id){
        $controller = get_class($this);
        $userdata = $this->session->userdata("userdata");
        $draw = $_REQUEST['draw']; // get the requested page 
        $start = $_REQUEST['start'];
...
function perkembangan_hapus(){
    ...
    $this->cm->set_unsync2("lap_laka_lantas","lap_laka_lantas_id","lap_a_perkembangan","id",$data['id']);
