...
function get_data_perkembangan($lap_d_id){
        $controller = get_class($this);
        $userdata = $this->session->userdata("userdata");
        $draw = $_REQUEST['draw']; // get the requested page 
        $start = $_REQUEST['start'];
...
function perkembangan_simpan(){
        ...
        if($this->form_validation->run() == TRUE ) { 
            ...
            //$data['user_id'] = $userdata['id'];
             ...
             $res = $this->db->insert("lap_d_perkembangan",$data);

             $lap_d_id = $this->db->insert_id();
...
function perkembangan_update(){
        ...
        if($this->form_validation->run() == TRUE ) { 
            ...
            $this->db->where("id",$data['id']);
             $res = $this->db->update("lap_d_perkembangan",$data);
...
function perkembangan_hapus(){
    ...
    $this->cm->set_unsync2("lap_d","lap_d_id","lap_d_perkembangan","id",$data['id']);

    $this->db->where("id",$data['id']);
    $res = $this->db->delete("lap_d_perkembangan");
    if($res){
