<?php
class analisa_sasaran_model extends CI_Model {
	function analisa_sasaran_model(){
		parent::__construct();
	}
...
function data($param){
	...
	$arr_column = array(
		...
		"tanggal","sasaran","kondisi");

	$sort_by = $arr_column[$param['sort_by']];

	$this->db->select('*')->from('analisa_sasaran');
...
function detail($id){
	$this->db->where("id",$id);
	$ret = $this->db->get('analisa_sasaran');
	return $ret;
}
