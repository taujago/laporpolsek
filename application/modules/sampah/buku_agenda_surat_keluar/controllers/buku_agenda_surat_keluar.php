<?php
class buku_agenda_surat_keluar extends master_controller {

	var $controller ;

	function buku_agenda_surat_keluar(){
		parent::__construct();
		// $this->load->model("core_model","cm");
		$this->load->model("coremodel","cm");
		$this->load->helper("tanggal");
		$this->load->model("basout_model","dm");
	...
	function index(){
		...
		$this->set_subtitle("DATA BUKU AGENDA SURAT KELUAR");
		$this->set_title("DATA BUKU AGENDA SURAT KELUAR");
...
"id" => "","sifat"=>""
		...
		$this->set_subtitle("INPUT DATA  BUKU AGENDA SURAT KELUAR");
		$this->set_title(" BUKU AGENDA SURAT KELUAR");
		$this->set_content($content);
		$this->render_baru();
...
function simpan(){
		...
		if($this->form_validation->run() == TRUE ) { 
			unset($data['mode']);
			unset($data['id']);
			 $res = $this->db->insert("buku_agenda_surat_keluar",$data);
...
function edit($id){
		...
		$this->set_subtitle("EDIT DATA BUKU AGENDA SURAT KELUAR");
		$this->set_title("EDIT DATA BUKU AGENDA SURAT KELUAR");
		$this->set_content($content);
		$this->render_baru();
	}
		...
		if($this->form_validation->run() == TRUE ) { 
 		...
 		$data['tanggal'] = flipdate($data['tanggal']);
			...
			$this->db->where("id",$data['id']);
			 $res = $this->db->update("buku_agenda_surat_keluar",$data);
			 if($res) {
...
function detail($id){
		...
		$this->set_subtitle("DETAIL DATA BUKU AGENDA SURAT KELUAR");
		$this->set_title("DETAIL DATA BUKU AGENDA SURAT KELUAR");
		$this->set_content($content);
		$this->render_baru();
...
function cetak_agenda($id){
		...
		$sifat = $this->uri->segment(5); 
	 	...
	 	$res = $this->db->get("buku_agenda_surat_keluar");
