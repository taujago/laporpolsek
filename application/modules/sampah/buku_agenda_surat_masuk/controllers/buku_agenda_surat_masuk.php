<?php
class buku_agenda_surat_masuk extends master_controller {

	var $controller ;

	function buku_agenda_surat_masuk(){
		parent::__construct();
		// $this->load->model("core_model","cm");
		$this->load->model("coremodel","cm");
		$this->load->helper("tanggal");
		$this->load->model("basin_model","dm");
	...
	function index(){
		...
		$this->set_subtitle("DATA BUKU AGENDA SURAT MASUK");
		$this->set_title("DATA BUKU AGENDA SURAT MASUK");
...
$id = $row['id'];
        	...
        	$arr_data[] = array(
        		  				...
        		  				flipdate($row['terima_tanggal']) ." ". $row['terima_jam'], 
        		  			 	$row['nomor']  . " " .flipdate($row['tanggal']),         		  				 
        		  				$row['asal_surat'], 
        		  				$row['perihal'], 
        		  				$row['keterangan'], 
        		  				$row['sifat'], 
...
"id" => "","sifat"=>""
		...
		$this->set_subtitle("INPUT DATA  BUKU AGENDA SURAT MASUK");
		$this->set_title(" BUKU AGENDA SURAT MASUK");
		$this->set_content($content);
		$this->render_baru();
...
function simpan(){
		...
		$this->load->library('form_validation');
 		$this->form_validation->set_rules('terima_tanggal','Tanggal terima','required');	
		...
		$this->form_validation->set_message('required', ' %s Harus diisi ');
 		...
 		$data['terima_tanggal'] = flipdate($data['terima_tanggal']);
 		$data['disposisi_tanggal'] = flipdate($data['disposisi_tanggal']);
		...
		if($this->form_validation->run() == TRUE ) { 
			unset($data['mode']);
			unset($data['id']);
			 $res = $this->db->insert("buku_agenda_surat_masuk",$data);
...
function edit($id){
		...
		$this->set_subtitle("EDIT DATA BUKU AGENDA SURAT MASUK");
		$this->set_title("EDIT DATA BUKU AGENDA SURAT MASUK");
		$this->set_content($content);
		$this->render_baru();
	}
		...
		if($this->form_validation->run() == TRUE ) { 
 		...
 		$data['disposisi_tanggal'] = flipdate($data['disposisi_tanggal']);
			...
			$this->db->where("id",$data['id']);
			 $res = $this->db->update("buku_agenda_surat_masuk",$data);
			 if($res) {
...
function hapus(){
	$data = $this->input->post();
	$this->db->where("id",$data['id']);
	$res = $this->db->delete("buku_agenda_surat_masuk");
	if($res){
...
function detail($id){
		...
		$this->set_subtitle("DETAIL DATA BUKU AGENDA SURAT MASUK");
		$this->set_title("DETAIL DATA BUKU AGENDA SURAT MASUK");
		$this->set_content($content);
		$this->render_baru();
...
function cetak_agenda($id){
		...
		$sifat = $this->uri->segment(5); 

	 
	 	$this->db->where("year(terima_tanggal) = $tahun",null,false);
	 	$this->db->where("month(terima_tanggal) = $bulan",null,false);
	 	$this->db->where("sifat",$sifat);
	 	$res = $this->db->get("buku_agenda_surat_masuk");
		...
		$data['rec_agenda'] = $res ;
		$data['sifat'] = $sifat ;
		//$data['terima_jam'] = substr($data['terima_jam'],0,5);
		$data['tahun'] = $tahun;
		$arr_bulan = arr_bulan();
		...
		$pdf->AddPage('L');
		$html = $this->load->view("pdf/buku_agenda_surat_masuk_agenda",$data,true);		 
		$pdf->writeHTML($html, true, false, true, false, '');
		 ...
		 $html = $this->load->view("pdf/buku_agenda_surat_masuk_ttd",$data,true);
		 $pdf->writeHTML($html, true, false, true, false, '');
		...
		$pdf->Output('Buku Agenda Surat Masuk.pdf', 'I');
