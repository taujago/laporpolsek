<?php
class bati_model extends CI_Model {
	function bati_model(){
		parent::__construct();
	}
...
function data($param){
	...
	$arr_column = array(
		...
		"tanggal","nomor","pembuat","perihal","lampiran","tindak_lanjut","keterangan");

	$sort_by = $arr_column[$param['sort_by']];

	$this->db->select('*')->from('buku_agenda_telaahan_intelijen');
...
function detail($id){
	$this->db->where("id",$id);
	$ret = $this->db->get('buku_agenda_telaahan_intelijen');
	return $ret;
}
