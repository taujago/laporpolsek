<?php
class buku_arsip extends master_controller {
	...
	function buku_arsip(){
		...
		$this->load->model("buku_arsip_model","dm");
	...
	function index(){
		...
		$this->set_subtitle("DATA BUKU ARSIP");
		$this->set_title("DATA BUKU ARSIP");
...
$id = $row['id'];
        	...
        	$arr_data[] = array(
        		  				...
        		  				$n, 
        		  				flipdate($row['tanggal']). " ".substr($row['jam'],0,5) ,         		  				 
        		  				$row['dari'], 
        		  				flipdate($row['surat_tanggal']) . "<br />" . $row['surat_nomor'], 
        		  				$row['perihal'], 
        		  				$row['lampiran'], 
        		  				$row['kode_penyimpanan'], 
        		  				$row['keterangan'],         		  				 
...
"id" => "","sifat"=>""
		...
		$this->set_subtitle("INPUT DATA  BUKU ARSIP");
		$this->set_title(" BUKU ARSIP");
		$this->set_content($content);
		$this->render_baru();
...
function simpan(){
		...
		$this->load->library('form_validation');
 		$this->form_validation->set_rules('surat_nomor','Nomor surat ','required');	 
		$this->form_validation->set_rules('surat_tanggal','Tanggal surat','required');	
		...
		if($this->form_validation->run() == TRUE ) { 
			unset($data['mode']);
			unset($data['id']);
			 $res = $this->db->insert("buku_arsip",$data);
...
function edit($id){
		...
		$this->set_subtitle("EDIT DATA BUKU ARSIP");
		$this->set_title("EDIT DATA BUKU ARSIP");
		$this->set_content($content);
		$this->render_baru();
	}
 		...
 		$data['tanggal'] = flipdate($data['tanggal']);
 	 	$data['surat_tanggal'] = flipdate($data['surat_tanggal']);
			...
			$this->db->where("id",$data['id']);
			 $res = $this->db->update("buku_arsip",$data);
			 if($res) {
...
function detail($id){
		...
		$this->set_subtitle("DETAIL DATA BUKU ARSIP");
		$this->set_title("DETAIL DATA BUKU ARSIP");
		$this->set_content($content);
		$this->render_baru();
