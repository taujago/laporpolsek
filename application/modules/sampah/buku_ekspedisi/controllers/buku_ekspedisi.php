<?php
class buku_ekspedisi extends master_controller {

	var $controller ;

	function buku_ekspedisi(){
		parent::__construct();
		// $this->load->model("core_model","cm");
		$this->load->model("coremodel","cm");
		$this->load->helper("tanggal");
		$this->load->model("ekspedisi_model","dm");
	...
	function index(){
		...
		$this->set_subtitle("DATA BUKU EKSPEDISI SURAT");
		$this->set_title("DATA BUKU EKSPEDISI SURAT");
...
"id" => "","sifat"=>""
		...
		$this->set_subtitle("INPUT DATA  BUKU EKSPEDISI SURAT");
		$this->set_title(" BUKU EKSPEDISI SURAT");
		$this->set_content($content);
		$this->render_baru();
...
function simpan(){
		...
		$this->form_validation->set_message('required', ' %s Harus diisi ');
 		...
 		$data['terima_waktu'] = flipdate($data['terima_waktu']);
		...
		if($this->form_validation->run() == TRUE ) { 
			unset($data['mode']);
			unset($data['id']);
			 $res = $this->db->insert("buku_ekspedisi",$data);
...
function edit($id){
		...
		$this->set_subtitle("EDIT DATA BUKU EKSPEDISI SURAT");
		$this->set_title("EDIT DATA BUKU EKSPEDISI SURAT");
		$this->set_content($content);
		$this->render_baru();
	}
		...
		if($this->form_validation->run() == TRUE ) { 
 		...
 		$data['terima_waktu'] = flipdate($data['terima_waktu']);
			...
			$this->db->where("id",$data['id']);
			 $res = $this->db->update("buku_ekspedisi",$data);
			 if($res) {
...
function detail($id){
		...
		$this->set_subtitle("DETAIL DATA BUKU EKSPEDISI SURAT");
		$this->set_title("DETAIL DATA BUKU EKSPEDISI SURAT");
		$this->set_content($content);
		$this->render_baru();
...
function cetak_agenda($id){
		...
		$sifat = $this->uri->segment(5); 
	 	...
	 	$res = $this->db->get("buku_ekspedisi");
		...
		$pdf->SetTitle('Buku EKSPEDISI SURAT');
		...
		$pdf->Output('Buku EKSPEDISI SURAT.pdf', 'I');
