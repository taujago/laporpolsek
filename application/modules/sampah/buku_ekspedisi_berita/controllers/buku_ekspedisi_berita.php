<?php
class buku_ekspedisi_berita extends master_controller {

	var $controller ;

	function buku_ekspedisi_berita(){
		parent::__construct();
		// $this->load->model("core_model","cm");
		$this->load->model("coremodel","cm");
		$this->load->helper("tanggal");
		$this->load->model("ekspedisi_berita_model","dm");
	...
	function index(){
		...
		$this->set_subtitle("DATA BUKU EKSPEDISI BERITA");
		$this->set_title("DATA BUKU EKSPEDISI BERITA");
...
"id" => "","sifat"=>""
		...
		$this->set_subtitle("INPUT DATA  BUKU EKSPEDISI BERITA");
		$this->set_title(" BUKU EKSPEDISI BERITA");
		$this->set_content($content);
		$this->render_baru();
...
function simpan(){
		...
		if($this->form_validation->run() == TRUE ) { 
			unset($data['mode']);
			unset($data['id']);
			 $res = $this->db->insert("buku_ekspedisi_berita",$data);
...
function edit($id){
		...
		$this->set_subtitle("EDIT DATA BUKU EKSPEDISI BERITA");
		$this->set_title("EDIT DATA BUKU EKSPEDISI BERITA");
		$this->set_content($content);
		$this->render_baru();
	}
		...
		if($this->form_validation->run() == TRUE ) { 
 		...
 		$data['tanggal'] = flipdate($data['tanggal']);
			...
			$this->db->where("id",$data['id']);
			 $res = $this->db->update("buku_ekspedisi_berita",$data);
			 if($res) {
...
function hapus(){
	$data = $this->input->post();
	$this->db->where("id",$data['id']);
	$res = $this->db->delete("buku_ekspedisi_berita");
	if($res){
...
function detail($id){
		...
		$this->set_subtitle("DETAIL DATA BUKU EKSPEDISI BERITA");
		$this->set_title("DETAIL DATA BUKU EKSPEDISI BERITA");
		$this->set_content($content);
		$this->render_baru();
...
function cetak_agenda($id){
		...
		$sifat = $this->uri->segment(5); 
	 	...
	 	$res = $this->db->get("buku_ekspedisi_berita");
		...
		$pdf->SetTitle('Buku EKSPEDISI BERITA');
		...
		$pdf->Output('Buku EKSPEDISI BERITA.pdf', 'I');
