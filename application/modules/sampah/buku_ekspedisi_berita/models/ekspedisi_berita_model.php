<?php
class ekspedisi_berita_model extends CI_Model {
	function ekspedisi_berita_model(){
		parent::__construct();
	}
...
function data($param){
	...
	$arr_column = array(
		...
		"tanggal","nomor","keterangan","sifat");

	$sort_by = $arr_column[$param['sort_by']];

	$this->db->select('*')->from('buku_ekspedisi_berita');
...
function detail($id){
	$this->db->where("id",$id);
	$ret = $this->db->get('buku_ekspedisi_berita');
	return $ret;
}
