<?php
class pengamanan_model extends CI_Model {
	function pengamanan_model(){
		parent::__construct();
	}
...
function data($param){
	...
	$this->db->select('*')->from('v_pengamanan');
...
function detail_rencana($id){
	$this->db->where("id",$id);
	$ret = $this->db->get('pengamanan_rencana');
	return $ret;
}

function detail_pelaksana($id){
	$this->db->where("id",$id);
	$ret = $this->db->get('pengamanan_pelaksana');
	return $ret;
}
