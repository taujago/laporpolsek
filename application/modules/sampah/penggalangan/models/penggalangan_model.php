<?php
class penggalangan_model extends CI_Model {
	function penggalangan_model(){
		parent::__construct();
	}
...
function data($param){
	...
	$this->db->select('*')->from('v_penggalangan');
...
function detail_rencana($id){
	$this->db->where("id",$id);
	$ret = $this->db->get('penggalangan_rencana');
	return $ret;
}

function detail_pelaksana($id){
	$this->db->where("id",$id);
	$ret = $this->db->get('penggalangan_pelaksana');
	return $ret;
}
